...
import { templateID, userID } from "../../data";
...
const UploadComponent = ({ sendParams }) => {
  ...
  const sendEmail = async () => {
    if (email) {
      if (transactionReceipt) {
        ...
        await emailjs.send("gmail", templateID, templateParams, userID);
  ...
  const signHash = () => {
    ...
    try {
      web3.eth.sign(message, metamaskAccount, (err, signature) => {
        ...
        storehash.methods.sendHash(ipfsHash).send(
          ...
          (error, transactionHash) => {
            ...
            setIsDisabled(false);
            openNotification(
              "You will be receiving transaction receipt shortly"
            );
          }
        );
      });
      ...
      >
        ...
        <div className="upload-content-0">
          <p className="upload-p">
            The very first thing to do as an issuer is to create and deploy
            smart contract-which will cost some ether. Note that this contract
            is immutable once deployed and will be transparent to all the peers
            in the network who have the contract address.
          </p>
          <p>
          ...
          <div className="upload-content-1-form">
            ...
            <Form form={form} layout="vertical">
              ...
              <Button
                ...
                className="upload-content-1-form-button"
              >
                {loading ? <Spin /> : "Upload"}
              </Button>
            </Form>
          ...
          {ipfsHash ? (
            <div className="upload-content-1-sign-perm">
              ...
              <div className="upload-title-content">
                ...
                <span
                  className="upload-content-1-sign-perm-link"
                  onClick={signHash}
                >
                  Metamask Account
                </span>
        ...
        >
          {transactionReceipt ? (
            Object.entries(transactionReceipt)
              .filter(
                (entries) => entries[0] !== "logs" && entries[0] !== "logsBloom"
              )
              .map((entries, i) => {
                return (
